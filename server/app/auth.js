 ...
 */
const modelWrapper = require('../models/modelWrappers.js');

// Session Key + Password Authentication
module.exports.password = function (body, callback) {
  var promise = modelWrapper.SessionInfo.get(body.session, body.password);
...
module.exports.userKey = function (body, callback) {
  ...
  var promise = modelWrapper.UserKey.get(body.session, user_key);
  promise.then(function (data) {
    if (data) {
      callback(true, data);
    } else {
